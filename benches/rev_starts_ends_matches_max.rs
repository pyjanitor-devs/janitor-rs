//! Old label HashMap versus bounded dense slots for reverse starts/ends/matches max.
...
impl Fixture {
    fn new(rows: usize, right_len: usize, width: usize, broad: bool) -> Self {
        ...
        Self {
            arr: (0..rows).map(|row| (row % 10_000) as i64).collect(),
            starts,
            ends,
...
fn old_max(f: &Fixture) -> Vec<(i64, i64)> {
    ...
    for row in 0..f.arr.len() {
        for item in f.starts[row]..f.ends[row] {
            if f.matches[tape] != 0 {
                let entry = values.entry(f.index[item]).or_insert((i64::MIN, -1));
...
fn dense_max(f: &Fixture) -> Vec<(i64, i64)> {
    ...
    let mut best = vec![i64::MIN; width];
    ...
    touched
        .into_iter()
        .map(|slot| (f.index[min_start + slot], rows[slot]))
        .collect()
}
fn bench(c: &mut Criterion) {
    let mut group = c.benchmark_group("max_rev_starts_ends_matches_old_vs_dense");
    ...
    for (name, rows, right_len) in [
        ...
        for (kind, width, broad) in [("narrow", 8, false), ("broad", 64, true)] {
            ...
            group.bench_with_input(
                ...
                &f,
                |b, x| b.iter(|| black_box(old_max(x))),
            );
            group.bench_with_input(
                BenchmarkId::new(format!("dense_slice_{kind}"), name),
                &f,
                |b, x| b.iter(|| black_box(dense_max(x))),
            );
        }
    }
    group.finish();
