//! Old HashMap versus dense ordinal state for reverse ends+matches product.
...
fn old_prod(arr: &[i64], index: &[i64], ends: &[usize], matches: &[i8]) -> Vec<(i64, i64)> {
    let mut values = HashMap::with_capacity(index.len());
    let mut n = 0;
    for (current, &end) in arr.iter().zip(ends) {
        for item in 0..end {
            if matches[n] != 0 {
                let total = values.entry(index[item]).or_insert(1_i64);
                *total = total.wrapping_mul(*current);
            }
            n += 1;
        }
    }
...
fn dense_prod(arr: &[i64], index: &[i64], ends: &[usize], matches: &[i8]) -> Vec<(i64, i64)> {
    let max_end = ends.iter().copied().max().unwrap_or(0);
    let mut seen = vec![false; max_end];
    let mut touched = Vec::new();
    let mut totals = vec![1_i64; max_end];
    let mut n = 0;
    for (current, &end) in arr.iter().zip(ends) {
        for item in 0..end {
            if matches[n] != 0 {
                ...
                totals[item] = totals[item].wrapping_mul(*current);
            }
            n += 1;
        }
    }
...
fn bench(c: &mut Criterion) {
    let mut group = c.benchmark_group("prod_rev_ends_matches_old_vs_dense");
    ...
    for (name, rows, right_len, width) in [
        ...
        group.bench_with_input(BenchmarkId::new("old_hashmap", name), &fixture, |b, f| {
            b.iter(|| black_box(old_prod(&f.arr, &f.index, &f.ends, &f.matches)))
        });
        group.bench_with_input(BenchmarkId::new("dense_ordinal", name), &fixture, |b, f| {
            b.iter(|| black_box(dense_prod(&f.arr, &f.index, &f.ends, &f.matches)))
        });
    }
    group.finish();
}
