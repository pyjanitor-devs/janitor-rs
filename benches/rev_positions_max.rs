//! Old label HashMap versus dense position slots for reverse positions max.
...
fn old_max(arr: &[i64], index: &[i64], positions: &[i64]) -> Vec<(i64, i64)> {
    ...
    for (row, &position) in positions.iter().enumerate() {
        ...
        let entry = values.entry(index[position]).or_insert((i64::MIN, -1));
        if arr[row] > entry.0 {
...
fn dense_max(arr: &[i64], index: &[i64], positions: &[i64]) -> Vec<(i64, i64)> {
    let width = positions
        .iter()
    ...
    let mut best_values = vec![i64::MIN; width];
...
fn bench(c: &mut Criterion) {
    let mut group = c.benchmark_group("max_rev_positions_old_vs_dense");
    ...
    for (name, rows, right_len, width) in [
        ...
        for sparse in [true, false] {
            ...
            group.bench_with_input(
                ...
                &fixture,
                |b, x| b.iter(|| black_box(old_max(&x.arr, &x.index, &x.positions))),
            );
            group.bench_with_input(
                BenchmarkId::new(format!("dense_ordinal_{kind}"), name),
                &fixture,
                |b, x| b.iter(|| black_box(dense_max(&x.arr, &x.index, &x.positions))),
            );
        }
    }
    group.finish();
