//! Old HashMap versus dense ordinal state for reverse ends+matches minimum.
//! ELI5: use the right row's ordinal as a direct slot, then translate it to
//! the potentially gapped label only when producing the result.
...
fn old_min(arr: &[i64], index: &[i64], ends: &[usize], matches: &[i8]) -> Vec<(i64, i64)> {
...
fn dense_min(arr: &[i64], index: &[i64], ends: &[usize], matches: &[i8]) -> Vec<(i64, i64)> {
...
fn bench(c: &mut Criterion) {
    let mut group = c.benchmark_group("min_rev_ends_matches_old_vs_dense");
    ...
    for (name, rows, right_len, width) in [
        ...
        group.bench_with_input(BenchmarkId::new("old_hashmap", name), &fixture, |b, f| {
            b.iter(|| black_box(old_min(&f.arr, &f.index, &f.ends, &f.matches)))
        });
        group.bench_with_input(BenchmarkId::new("dense_ordinal", name), &fixture, |b, f| {
            b.iter(|| black_box(dense_min(&f.arr, &f.index, &f.ends, &f.matches)))
        });
    }
    group.finish();
}
