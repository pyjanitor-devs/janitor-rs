...
macro_rules! bin_search {
    ($fname:ident, $type:ty) => {
        /// For every `left[i]`, the first position in all of `right`
        /// whose value is greater than or equal to `left[i]`. `right` is
        /// assumed sorted ascending, which in particular means NaN-free:
        /// for a `right` that contains NaN, this function will not
        /// panic, but its result is not guaranteed to be the same for a
        /// contiguous `right` array as for a non-contiguous (e.g. sliced
        /// with a step) one. See `binary_search_le_first_core`'s doc
...
bin_search!(binary_search_le_first_int64, i64);
bin_search!(binary_search_le_first_int32, i32);
bin_search!(binary_search_le_first_int16, i16);
bin_search!(binary_search_le_first_int8, i8);
bin_search!(binary_search_le_first_uint64, u64);
bin_search!(binary_search_le_first_uint32, u32);
bin_search!(binary_search_le_first_uint16, u16);
bin_search!(binary_search_le_first_uint8, u8);
bin_search!(binary_search_le_first_f64, f64);
bin_search!(binary_search_le_first_f32, f32);
...
pub(crate) fn register(m: &Bound<'_, PyModule>) -> PyResult<()> {
    m.add_function(wrap_pyfunction!(binary_search_le_first_uint64, m)?)?;
    m.add_function(wrap_pyfunction!(binary_search_le_first_uint32, m)?)?;
    m.add_function(wrap_pyfunction!(binary_search_le_first_uint16, m)?)?;
    m.add_function(wrap_pyfunction!(binary_search_le_first_uint8, m)?)?;
    m.add_function(wrap_pyfunction!(binary_search_le_first_int64, m)?)?;
    m.add_function(wrap_pyfunction!(binary_search_le_first_int32, m)?)?;
    m.add_function(wrap_pyfunction!(binary_search_le_first_int16, m)?)?;
    m.add_function(wrap_pyfunction!(binary_search_le_first_int8, m)?)?;
