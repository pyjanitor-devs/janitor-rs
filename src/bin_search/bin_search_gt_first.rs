...
macro_rules! bin_search {
    ($fname:ident, $type:ty) => {
        ...
        /// whose value is strictly less than `left[i]`, returned as one
        /// past that position. `right` is assumed sorted ascending,
        ...
        /// `binary_search_gt_first_core`'s doc comment (in the Rust
        /// source) for why.
        #[pyfunction]
        pub fn $fname<'py>(
            py: Python<'py>,
...
mod tests {
    ...
    fn nan_in_right_does_not_panic_but_parity_is_not_guaranteed() {
        ...
        let left_index = array![0_i64];
        let (fast_indices, _) =
            binary_search_gt_first_core(left.view(), right_dense.view(), left_index.view());
        let (fallback_indices, _) =
            binary_search_gt_first_core(left.view(), right_strided, left_index.view());
