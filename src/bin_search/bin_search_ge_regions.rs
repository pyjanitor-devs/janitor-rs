...
macro_rules! bin_search {
    ($fname:ident, $type:ty) => {
        ...
        /// (unlike `binary_search_ge_core`, which searches a per-row
        /// `[start, end)` slice) for the first position whose value is
        /// greater than `left[i]`, then reports how many positions from
        /// there to the end of `right` there are -- dropping rows where
        ...
        pub fn $fname<'py>(
            ...
            for (pos, left_value) in left.into_iter().enumerate() {
                let min_idx = if let Some(slice) = right_slice {
                    ...
                    // (see bin_search_ge.rs's core for the full
...
bin_search!(binary_search_ge_regions_int64, i64);
bin_search!(binary_search_ge_regions_int32, i32);
bin_search!(binary_search_ge_regions_int16, i16);
bin_search!(binary_search_ge_regions_int8, i8);
bin_search!(binary_search_ge_regions_uint64, u64);
bin_search!(binary_search_ge_regions_uint32, u32);
bin_search!(binary_search_ge_regions_uint16, u16);
bin_search!(binary_search_ge_regions_uint8, u8);
bin_search!(binary_search_ge_regions_f64, f64);
bin_search!(binary_search_ge_regions_f32, f32);
...
pub(crate) fn register(m: &Bound<'_, PyModule>) -> PyResult<()> {
    m.add_function(wrap_pyfunction!(binary_search_ge_regions_uint64, m)?)?;
    m.add_function(wrap_pyfunction!(binary_search_ge_regions_uint32, m)?)?;
    m.add_function(wrap_pyfunction!(binary_search_ge_regions_uint16, m)?)?;
    m.add_function(wrap_pyfunction!(binary_search_ge_regions_uint8, m)?)?;
    m.add_function(wrap_pyfunction!(binary_search_ge_regions_int64, m)?)?;
    m.add_function(wrap_pyfunction!(binary_search_ge_regions_int32, m)?)?;
    m.add_function(wrap_pyfunction!(binary_search_ge_regions_int16, m)?)?;
    m.add_function(wrap_pyfunction!(binary_search_ge_regions_int8, m)?)?;
    m.add_function(wrap_pyfunction!(binary_search_ge_regions_f32, m)?)?;
    m.add_function(wrap_pyfunction!(binary_search_ge_regions_f64, m)?)?;
    Ok(())
}
...
mod tests {
    ...
    fn contiguous_and_strided_paths_agree() {
        ...
        Python::attach(|py| {
            ...
            let (fast_search, fast_left, fast_total) = binary_search_ge_regions_int64(
                py,
                left.readonly(),
                right_dense.readonly(),
                left_index.readonly(),
            );
            let (fallback_search, fallback_left, fallback_total) = binary_search_ge_regions_int64(
                py,
                left.readonly(),
                right_strided,
                left_index.readonly(),
    ...
    fn contiguous_and_strided_paths_agree_when_the_query_is_nan() {
        ...
        Python::attach(|py| {
            ...
            let (fast_search, fast_left, fast_total) = binary_search_ge_regions_f64(
                py,
                left.readonly(),
                right_dense.readonly(),
                left_index.readonly(),
            );
            let (fallback_search, fallback_left, fallback_total) = binary_search_ge_regions_f64(
                py,
                left.readonly(),
                right_strided,
                left_index.readonly(),
    ...
    fn nan_in_right_does_not_panic_but_parity_is_not_guaranteed() {
        ...
        Python::attach(|py| {
            ...
            let (fast_search, ..) = binary_search_ge_regions_f64(
                py,
                left.readonly(),
                right_dense.readonly(),
                left_index.readonly(),
            );
            let (fallback_search, ..) = binary_search_ge_regions_f64(
                py,
                left.readonly(),
                right_strided,
                left_index.readonly(),
