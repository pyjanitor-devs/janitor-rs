...
pub fn binary_search_ge_core<T: PartialOrd + Copy>(
    ...
    for (pos, (left_value, start, end)) in zipped.enumerate() {
        ...
        // direction -- `current_value > left_value` staying on the "max"
        // side is the same split as `current_value <= left_value` staying
        // on the "min" side), so it's a drop-in replacement, not a
        // different algorithm -- the fast path exists because a `&[T]`
        // slice gives the compiler bounds-check-elision and vectorization
...
mod tests {
    ...
    fn contiguous_and_strided_paths_agree() {
        ...
        let fallback = binary_search_ge_core(left.view(), right_strided, starts.view(), ends.view());
