...
/// Find, for every `left[i]`, one past the last position in
/// `right[starts[i]..ends[i])` whose value is strictly less than
/// `left[i]` (`right` is assumed sorted ascending within each
...
macro_rules! bin_search {
    ($fname:ident, $type:ty) => {
        /// For every `left[i]`, one past the last position in
        /// `right[starts[i]..ends[i])` whose value is strictly less than
        /// `left[i]`. `right` is assumed sorted ascending within each
        /// `[start, end)` slice, which in particular means NaN-free: for a
        /// `right` that contains NaN, this function will not panic, but
        /// its result is not guaranteed to be the same for a contiguous
        /// `right` array as for a non-contiguous (e.g. sliced with a
        /// step) one. See `binary_search_gt_core`'s doc comment (in the
        /// Rust source) for why.
        #[pyfunction]
        pub fn $fname<'py>(
            py: Python<'py>,
