...
/// Pure-Rust reverse-min core for the "no range" (equi-join) shape: for
...
pub fn min_rev_no_range_core<T>(
    arr: ArrayView1<T>,
    ...
    T: Copy + Default + PartialOrd,
{
    ...
    for (index_left, index_right) in zipped {
        // ELI5: see `max_rev/max_no_range.rs`'s identical guard for the
        // full rationale (`index_left` needs a bound check, `right_index`
        // doesn't).
...
macro_rules! compute {
    ($fname:ident, $type:ty) => {
        ...
        // The macro will expand into the contents of this block.
        {
            ...
            let (indexers, result) = min_rev_no_range_core(
                arr.as_array(),
                left_index.as_array(),
...
mod correctness_tests {
    ...
    use super::compute_min_rev_no_range_int64;
    ...
    fn touched_row_positions_are_emitted_ascending_with_winning_row_index() {
        ...
        Python::attach(|py| {
            ...
            // Row 0 (val 10) and row 2 (val 20) both feed group 1; row 0
            // wins (min). Row 1 (val 30) is the only feeder for group 0.
