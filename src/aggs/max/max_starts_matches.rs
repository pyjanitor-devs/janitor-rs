...
///
/// ELI5 (the guard): `max` needs a real array element to seed its
...
pub fn max_start_match_core<T: PartialOrd + Copy>(
    arr: ArrayView1<T>,
    ...
    ($fname:ident, $type:ty) => {
        ...
        // The macro will expand into the contents of this block.
        {
            ...
            let expected_matches_width: usize = starts
                .as_array()
                .iter()
                .filter_map(|s| checked_index(*s, end_).map(|s_| end_ - s_))
                .sum();
            ensure_tape_width(expected_matches_width, matches.as_array().len())?;
            let result = max_start_match_core(
                arr.as_array(),
                starts.as_array(),
                counts.as_array(),
                matches.as_array(),
...
generic_compute!(compute_max_start_match_int64, i64);
generic_compute!(compute_max_start_match_int32, i32);
generic_compute!(compute_max_start_match_int16, i16);
generic_compute!(compute_max_start_match_int8, i8);
generic_compute!(compute_max_start_match_uint64, u64);
generic_compute!(compute_max_start_match_uint32, u32);
generic_compute!(compute_max_start_match_uint16, u16);
generic_compute!(compute_max_start_match_uint8, u8);
generic_compute!(compute_max_start_match_f64, f64);
generic_compute!(compute_max_start_match_f32, f32);
...
pub(crate) fn register(m: &Bound<'_, PyModule>) -> PyResult<()> {
    m.add_function(wrap_pyfunction!(compute_max_start_match_uint64, m)?)?;
    m.add_function(wrap_pyfunction!(compute_max_start_match_uint32, m)?)?;
    m.add_function(wrap_pyfunction!(compute_max_start_match_uint16, m)?)?;
    m.add_function(wrap_pyfunction!(compute_max_start_match_uint8, m)?)?;
    m.add_function(wrap_pyfunction!(compute_max_start_match_int64, m)?)?;
    m.add_function(wrap_pyfunction!(compute_max_start_match_int32, m)?)?;
    m.add_function(wrap_pyfunction!(compute_max_start_match_int16, m)?)?;
    m.add_function(wrap_pyfunction!(compute_max_start_match_int8, m)?)?;
    m.add_function(wrap_pyfunction!(compute_max_start_match_f32, m)?)?;
    m.add_function(wrap_pyfunction!(compute_max_start_match_f64, m)?)?;
    Ok(())
}
