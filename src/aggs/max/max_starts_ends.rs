...
pub fn max_start_end_core<T: PartialOrd + Copy>(
    ...
    max_start_end_core_impl(arr, starts, ends, Some(booleans))
}

/// Find range maxima when the caller guarantees that every source value is
/// non-null. This avoids allocating an all-false mask for already filtered
/// range-join inputs.
pub(crate) fn max_start_end_core_no_nulls<T: PartialOrd + Copy>(
    arr: ArrayView1<T>,
    starts: ArrayView1<i64>,
    ends: ArrayView1<i64>,
) -> Result<Array1<i64>, String> {
    max_start_end_core_impl(arr, starts, ends, None)
}

fn max_start_end_core_impl<T: PartialOrd + Copy>(
    arr: ArrayView1<T>,
    starts: ArrayView1<i64>,
    ends: ArrayView1<i64>,
...
mod tests {
    ...
    fn segment_tree_preserves_maximum_positions_and_nulls() {
        ...
        let got =
            max_start_end_core(arr.view(), starts.view(), ends.view(), booleans.view()).unwrap();
        assert_eq!(got, Array1::from_elem(16, 16_i64));
