...
pub fn min_start_end_core<T: PartialOrd + Copy>(
    arr: ArrayView1<T>,
...
macro_rules! generic_compute {
    ($fname:ident, $type:ty) => {
        /// Finds the position of the smallest value in each half-open
...
generic_compute!(compute_min_start_end_int64, i64);
generic_compute!(compute_min_start_end_int32, i32);
generic_compute!(compute_min_start_end_int16, i16);
generic_compute!(compute_min_start_end_int8, i8);
generic_compute!(compute_min_start_end_uint64, u64);
generic_compute!(compute_min_start_end_uint32, u32);
generic_compute!(compute_min_start_end_uint16, u16);
generic_compute!(compute_min_start_end_uint8, u8);
generic_compute!(compute_min_start_end_f64, f64);
generic_compute!(compute_min_start_end_f32, f32);
...
pub(crate) fn register(m: &Bound<'_, PyModule>) -> PyResult<()> {
    m.add_function(wrap_pyfunction!(compute_min_start_end_uint64, m)?)?;
    m.add_function(wrap_pyfunction!(compute_min_start_end_uint32, m)?)?;
    m.add_function(wrap_pyfunction!(compute_min_start_end_uint16, m)?)?;
    m.add_function(wrap_pyfunction!(compute_min_start_end_uint8, m)?)?;
    m.add_function(wrap_pyfunction!(compute_min_start_end_int64, m)?)?;
    m.add_function(wrap_pyfunction!(compute_min_start_end_int32, m)?)?;
    m.add_function(wrap_pyfunction!(compute_min_start_end_int16, m)?)?;
    m.add_function(wrap_pyfunction!(compute_min_start_end_int8, m)?)?;
    m.add_function(wrap_pyfunction!(compute_min_start_end_f32, m)?)?;
    m.add_function(wrap_pyfunction!(compute_min_start_end_f64, m)?)?;
    Ok(())
}
...
mod tests {
    ...
    #[test]
    fn finds_position_of_smallest_in_interior_slice() {
    ...
    fn adaptive_tree_skips_nan_values_marked_null() {
        let arr = array![2.0_f64, f64::NAN, 1.0];
