...
/// #27.
pub fn min_start_core<T: PartialOrd + Copy>(
    arr: ArrayView1<T>,
...
pub(crate) fn register(m: &Bound<'_, PyModule>) -> PyResult<()> {
    m.add_function(wrap_pyfunction!(compute_min_start_uint64, m)?)?;
    m.add_function(wrap_pyfunction!(compute_min_start_uint32, m)?)?;
    m.add_function(wrap_pyfunction!(compute_min_start_uint16, m)?)?;
    m.add_function(wrap_pyfunction!(compute_min_start_uint8, m)?)?;
    m.add_function(wrap_pyfunction!(compute_min_start_int64, m)?)?;
    m.add_function(wrap_pyfunction!(compute_min_start_int32, m)?)?;
    m.add_function(wrap_pyfunction!(compute_min_start_int16, m)?)?;
    m.add_function(wrap_pyfunction!(compute_min_start_int8, m)?)?;
    m.add_function(wrap_pyfunction!(compute_min_start_f32, m)?)?;
    m.add_function(wrap_pyfunction!(compute_min_start_f64, m)?)?;
    Ok(())
}
...
mod tests {
    ...
    fn empty_array_is_rejected() {
        ...
        let error = min_start_core(arr.view(), starts.view(), booleans.view()).unwrap_err();
    ...
    #[test]
    fn finds_position_of_smallest_in_suffix() {
