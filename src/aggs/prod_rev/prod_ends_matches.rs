    ...
    F: FnMut(T) -> A,
{
        ...
        izip!(arr.iter(), ends.iter(), counts.iter(), booleans.iter())
    {
        ...
        for item in 0..end_ {
            if matches[tape] != 0 {
                let total = totals.entry(item).or_insert(A::ONE);
                if !*boolean && *count != 0 {
    ...
    F: FnMut(T) -> f64,
{
        ...
        izip!(arr.iter(), ends.iter(), counts.iter(), booleans.iter())
    {
        ...
        for item in 0..end_ {
            if matches[tape] != 0 {
                let total = totals.entry(item).or_insert(1.);
                if !*boolean && *count != 0 {
                    *total *= current_;
                }
            }
...
mod precedence_tests {
    use super::prod_rev_end_match_int_core;
    use numpy::ndarray::{array, Array1};

    #[test]
    fn empty_input_precedes_shape_mismatch() {
        let error = prod_rev_end_match_int_core::<i64, i64, _>(
