...
#[pyfunction(name = "compute_sum_rev_start_end_match_int64")]
pub fn compute_int64<'py>(
    ...
    let (indexers, result) =
        array_compute_int64(arr, starts, ends, index, counts, matches, booleans);

    (indexers.into_pyarray(py), result.into_pyarray(py))
}

#[pyfunction(name = "compute_sum_rev_start_end_match_int32")]
pub fn compute_int32<'py>(
    ...
    let booleans = booleans.as_array();
    let (indexers, result) =
        array_compute_int32(arr, starts, ends, index, counts, matches, booleans);

    (indexers.into_pyarray(py), result.into_pyarray(py))
}

#[pyfunction(name = "compute_sum_rev_start_end_match_int16")]
pub fn compute_int16<'py>(
    ...
    let booleans = booleans.as_array();
    let (indexers, result) =
        array_compute_int16(arr, starts, ends, index, counts, matches, booleans);

    (indexers.into_pyarray(py), result.into_pyarray(py))
}

#[pyfunction(name = "compute_sum_rev_start_end_match_int8")]
pub fn compute_int8<'py>(
    ...
    let booleans = booleans.as_array();
    let (indexers, result) =
        array_compute_int8(arr, starts, ends, index, counts, matches, booleans);

    (indexers.into_pyarray(py), result.into_pyarray(py))
}

#[pyfunction(name = "compute_sum_rev_start_end_match_uint64")]
pub fn compute_uint64<'py>(
    ...
    let booleans = booleans.as_array();
    let (indexers, result) =
        array_compute_uint64(arr, starts, ends, index, counts, matches, booleans);

    (indexers.into_pyarray(py), result.into_pyarray(py))
}

#[pyfunction(name = "compute_sum_rev_start_end_match_uint32")]
pub fn compute_uint32<'py>(
    ...
    let booleans = booleans.as_array();
    let (indexers, result) =
        array_compute_uint32(arr, starts, ends, index, counts, matches, booleans);

    (indexers.into_pyarray(py), result.into_pyarray(py))
}

#[pyfunction(name = "compute_sum_rev_start_end_match_uint16")]
pub fn compute_uint16<'py>(
    ...
    let booleans = booleans.as_array();
    let (indexers, result) =
        array_compute_uint16(arr, starts, ends, index, counts, matches, booleans);

    (indexers.into_pyarray(py), result.into_pyarray(py))
}

#[pyfunction(name = "compute_sum_rev_start_end_match_uint8")]
pub fn compute_uint8<'py>(
    ...
    let booleans = booleans.as_array();
    let (indexers, result) =
        array_compute_uint8(arr, starts, ends, index, counts, matches, booleans);

    (indexers.into_pyarray(py), result.into_pyarray(py))
}

#[pyfunction(name = "compute_sum_rev_start_end_match_f32")]
pub fn compute_f32<'py>(
    ...
    let matches = matches.as_array();
    let booleans = booleans.as_array();
    let (indexers, result) = array_compute_f32(arr, starts, ends, index, counts, matches, booleans);

    (indexers.into_pyarray(py), result.into_pyarray(py))
}

#[pyfunction(name = "compute_sum_rev_start_end_match_f64")]
pub fn compute_f64<'py>(
    ...
    let matches = matches.as_array();
    let booleans = booleans.as_array();
    let (indexers, result) = array_compute_f64(arr, starts, ends, index, counts, matches, booleans);
