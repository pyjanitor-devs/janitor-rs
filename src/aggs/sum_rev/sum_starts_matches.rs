...
/// Aggregate integer sums for reverse suffixes selected by a survivor tape.
///
/// `arr` must not be empty; `matches` must contain one entry per candidate
/// position in the supplied suffix tape. Null rows and rows with zero counts
...
/// Aggregate floating-point sums for reverse suffixes selected by a survivor
/// tape.
///
/// `arr` must not be empty; `matches` must contain one entry per candidate
/// position in the supplied suffix tape. Null rows and rows with zero counts
    ...
    F: FnMut(T) -> f64,
{
        ...
        izip!(arr.iter(), starts.iter(), counts.iter(), booleans.iter())
    {
        ...
        for item in start_..index.len() {
            if matches[tape] != 0 {
                let slot = item - min_start;
                let state = states.entry(slot).or_insert((0., 0.));
                if !*boolean && *count != 0 {
                    let difference = current_ - state.1;
                    let increment = state.0 + difference;
                    state.1 = (increment - state.0) - difference;
    ...
    for (slot, (total, _compensation)) in states {
        labels.push(index[min_start + slot]);
        values.push(total);
    }
    Ok((labels, values))
...
mod precedence_tests {
    use super::sum_rev_start_match_int_core;
    use numpy::ndarray::{array, Array1};

    #[test]
    fn empty_input_precedes_shape_mismatch() {
        let error = sum_rev_start_match_int_core::<i64, i64, _>(
