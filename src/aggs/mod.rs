...
/// Reject an input array that cannot describe any work.
pub(crate) fn ensure_nonempty(name: &str, len: usize) -> Result<(), &'static str> {
    if len == 0 {
        let _ = name;
        return Err("required aggregation inputs cannot be empty");
    }
    Ok(())
}

/// Validate index-like values before they are converted to `usize`.
pub(crate) fn ensure_valid_indices(
    name: &str,
    ...
    {
        let _ = (name, len);
        return Err("index values must be non-negative and within the input length");
    }
    Ok(())
}

/// Validate inclusive-exclusive end bounds. `0` is valid for an end because
/// it describes an empty prefix; `len` is valid because it is a slice bound.
pub(crate) fn ensure_valid_ends(
    name: &str,
    values: impl IntoIterator<Item = i64>,
    len: usize,
) -> Result<(), &'static str> {
    ...
    {
        let _ = (name, len);
        return Err("end values must be non-negative and no greater than the input length");
    }
    Ok(())
}

/// Validate non-empty half-open ranges. Equal bounds are rejected because an
/// individual empty query is not a meaningful aggregation request.
pub(crate) fn ensure_valid_ranges(
    ...
    for (start, end) in starts.into_iter().zip(ends) {
        let Some(start) = usize::try_from(start).ok() else {
            return Err("range bounds must be non-negative");
        };
        ...
        if end > len || start >= end {
            let _ = len;
            return Err("ranges must satisfy 0 <= start < end <= input length");
        }
    }
    Ok(())
}
