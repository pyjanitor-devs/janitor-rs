...
/// Run maximum positional aggregation with an explicit storage mode.
///
...
macro_rules! compute {
    ($fname:ident, $type:ty) => {
        /// Find the maximum contributing row position for each label reached
...
compute!(compute_max_rev_positions_int64, i64);
compute!(compute_max_rev_positions_int32, i32);
compute!(compute_max_rev_positions_int16, i16);
compute!(compute_max_rev_positions_int8, i8);
compute!(compute_max_rev_positions_uint64, u64);
compute!(compute_max_rev_positions_uint32, u32);
compute!(compute_max_rev_positions_uint16, u16);
compute!(compute_max_rev_positions_uint8, u8);
compute!(compute_max_rev_positions_f64, f64);
compute!(compute_max_rev_positions_f32, f32);
...
pub(crate) fn register(m: &Bound<'_, PyModule>) -> PyResult<()> {
    m.add_function(wrap_pyfunction!(compute_max_rev_positions_uint64, m)?)?;
    m.add_function(wrap_pyfunction!(compute_max_rev_positions_uint32, m)?)?;
    m.add_function(wrap_pyfunction!(compute_max_rev_positions_uint16, m)?)?;
    m.add_function(wrap_pyfunction!(compute_max_rev_positions_uint8, m)?)?;
    m.add_function(wrap_pyfunction!(compute_max_rev_positions_int64, m)?)?;
    m.add_function(wrap_pyfunction!(compute_max_rev_positions_int32, m)?)?;
    m.add_function(wrap_pyfunction!(compute_max_rev_positions_int16, m)?)?;
    m.add_function(wrap_pyfunction!(compute_max_rev_positions_int8, m)?)?;
    m.add_function(wrap_pyfunction!(compute_max_rev_positions_f32, m)?)?;
    m.add_function(wrap_pyfunction!(compute_max_rev_positions_f64, m)?)?;
    Ok(())
}
...
mod tests {
    ...
    fn positions_keep_labels_and_maximum_rows() {
        ...
        got.sort_unstable();
        assert_eq!(got, vec![(10, 0), (20, 0), (30, 1)]);
    }

    #[test]
    fn null_rows_create_labels_but_do_not_select_a_maximum() {
        let arr = array![7_i64, 9];
